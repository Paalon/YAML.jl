...
construct_undefined_json_schema(constructor::Constructor, node::Node) =
    throw(ConstructorError("could not determine a constructor for the tag '$(node.tag)' in the JSON schema", firstmark(node)))
...
function construct_json_schema_null(constructor::Constructor, node::Node)::Nothing
    ...
    throw(ConstructorError("could not construct a null '$str' in the JSON schema", firstmark(node)))
    n
end
...
function construct_json_schema_bool(constructor::Constructor, node::Node)::Bool
    ...
    throw(ConstructorError("could not construct a bool '$str' in the JSON schema", firstmark(node)))
    b
end
...
function construct_json_schema_int(constructor::Constructor, node::Node)::Int
    ...
    n isa JSONSchemaParseError &&
    throw(ConstructorError("could not construct an int '$str' in the JSON schema", firstmark(node)))
    n
end
...
function construct_json_schema_float(construct::Constructor, node::Node)::Float64
    ...
    throw(ConstructorError("could not construct a float '$str' in the JSON schema", firstmark(node)))
    x
end
