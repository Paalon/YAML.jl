...
construct_undefined_yaml_jl_0_4_10_schema(constructor::Constructor, node::Node) =
    throw(ConstructorError("could not determine a constructor for the tag '$(node.tag)' the YAML.jl v0.4.0 schema", firstmark(node)))
...
function construct_yaml_jl_0_4_10_schema_int(constructor::Constructor, node::Node)
    ...
    if in(':', str)
        # TODO:
        # throw(ConstructorError("sexagesimal integers not yet implemented", firstmark(node)))
...
function construct_yaml_jl_0_4_10_schema_float(constructor::Constructor, node::Node)
    ...
    if in(':', str)
        # TODO:
        # throw(ConstructorError("sexagesimal floats not yet implemented", firstmark(node)))
...
construct_yaml_jl_0_4_10_schema_omap(constructor::Constructor, node::Node) =
    throw(ConstructorError("omap type not yet implemented", firstmark(node)))

construct_yaml_jl_0_4_10_schema_pairs(constructor::Constructor, node::Node) =
    throw(ConstructorError("pairs type not yet implemented", firstmark(node)))

construct_yaml_jl_0_4_10_schema_set(constructor::Constructor, node::Node) =
    throw(ConstructorError("set type not yet implemented", firstmark(node)))

construct_yaml_jl_0_4_10_schema_object(constructor::Constructor, node::Node) =
    throw(ConstructorError("object type not yet implemented", firstmark(node)))
