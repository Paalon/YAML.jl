...
construct_undefined_core_schema(constructor::Constructor, node::Node) =
    throw(ConstructorError("could not determine a constructor for the tag '$(node.tag)' in the Core schema", firstmark(node)))
...
function construct_core_schema_null(constructor::Constructor, node::Node)::Nothing
    ...
    throw(ConstructorError("could not construct a null '$str' in the Core schema", firstmark(node)))
    n
end
...
function construct_core_schema_bool(constructor::Constructor, node::Node)::Bool
    ...
    throw(ConstructorError("could not construct a bool '$str' in the Core schema", firstmark(node)))
    b
end
...
function construct_core_schema_int(constructor::Constructor, node::Node)::Int
    ...
    n isa CoreSchemaParseError &&
    throw(ConstructorError("could not construct an int '$str' in the Core schema", firstmark(node)))
    n
end
...
function construct_core_schema_float(construct::Constructor, node::Node)::Float64
    ...
    throw(ConstructorError("could not construct a float '$str' in the Core schema", firstmark(node)))
    x
end
