...
function construct_object(constructor::Constructor, node::Node)
    ...
    node in constructor.recursive_objects && throw(ConstructorError(
        "found unconstructable recursive node", firstmark(node),
    ))
...
function construct_scalar(constructor::Constructor, node::Node)
    node isa ScalarNode || throw(ConstructorError(
        "expected a scalar node, but found $(typeof(node))", firstmark(node),
    ))
    node.value
end
...
function construct_sequence(constructor::Constructor, node::Node)
    node isa SequenceNode || throw(ConstructorError(
        "expected a sequence node, but found $(typeof(node))", firstmark(node),
    ))
            ...
            elseif value_node isa SequenceNode
                ...
                for subnode in value_node.value
                    subnode isa MappingNode || throw(ConstructorError(
                        "while constructing a mapping", firstmark(node),
                        "expected a mapping node, but found $(typeof(subnode))", firstmark(subnode),
                    ))
    ...
    for (key_node, value_node) in node.value
        ...
        if !(value isa keytype(mapping))
            try
                ...
                throw(ConstructorError("Cannot cast $key to the key type of $dicttype", firstmark(node)))
            end
        end
        try
            mapping[key] = value
        catch
            throw(ConstructorError("Cannot store $key=>$value in $dicttype", firstmark(node)))
        end
    end
    mapping
end
